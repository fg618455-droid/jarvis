...
> **Backend abstraction and lanes.** Every completion below enters `get_llm_backend(cfg)` and selects FAST, CHAT, or PRIVATE through `resolve_model(cfg, tier)`. Configured FAST and CHAT chains are exactly the generic endpoint routes configured for them; no local model is appended behind either, so an unreachable chain fails the turn rather than substituting a local reply. PRIVATE contains loopback Ollama only. `get_embedding_backend(cfg)` is separate, always local Ollama, and never routed. FCC-imported Gemini and OpenRouter routes use the generic OpenAI-compatible wire shape and may contribute to CHAT only after their live `GET /models` response advertises at least one model. Their CHAT calls carry the same context-specific inputs shown below to the selected remote endpoint; neither route participates in FAST or PRIVATE. A configured CHAT route may use the `claude_subscription` provider: the main process sends a text-generation-only request over local newline-delimited JSON pipes to a lazy sidecar running under `~/.jarvis/claude-subscription-venv`. Only that subprocess imports `claude_agent_sdk` and authenticates through Felix's Claude Code subscription login; the main interpreter retains `mcp==1.13.1`. A missing or failed sidecar is an ordinary typed route failure, so the CHAT chain continues unchanged (see `src/jarvis/llm/llm.spec.md`, "Claude subscription session"). A configured CHAT route may use `codex_subscription`: a fresh read-only `codex exec` process authenticated by Felix's ChatGPT subscription, with user configuration ignored and a fresh empty temporary working directory. Codex routes follow faster configured CHAT candidates. A configured CHAT route may instead (or additionally) use the `crew_chat` provider: a text-generation-only HTTP relay to the Hermes crew's own chat engine on Felix's NAS (`cfg.crew_api_url`), a synchronous alternative to `askCrew`'s fire-and-forget Telegram delegation and a wholly independent code path from it (see `src/jarvis/llm/llm.spec.md`, "Crew chat relay"). `claude_subscription`, `codex_subscription`, and `crew_chat` never appear in a PRIVATE chain; Codex is also excluded from FAST.
>
...
> capabilities, so saving a route does not change the data flow before the
> selected backend is built.
...
- **Model / gating**: CHAT tier. The route-specific model and timeout come from the ordered CHAT chain. Not optional. No size branching on the loop itself; size branching affects the digests around it. Which route answers is decided per turn by two different mechanisms. `cfg.chat_backend_override` (`"auto"` by default, or a route provider such as `"claude_subscription"`, `"codex_subscription"`, or `"crew_chat"`) is a **pin**: it travels as `forced_provider` and removes the rest of the chain for that call, so the named provider answers or the turn fails with a message naming it. Under `"auto"`, the tool router's own classification (see #7) travels as `preferred_provider`, which only reorders the existing chain, so an unavailable or failing preferred route still falls through to the normal order. A tool-bearing call pinned to a provider that advertises no native tools raises `ToolsNotSupportedError`, and the loop re-asks that same backend with markdown-fenced tools rather than leaving it. When the resolved route is `claude_subscription`, the backend crosses a local subprocess pipe into the isolated SDK environment before the SDK reaches the Claude subscription service; sidecar readiness and the whole generation share the route timeout. When it is `codex_subscription`, the route model is passed to `codex exec --model`, the prompt crosses stdin, and completed JSONL agent messages return over stdout. The process timeout is the smaller of its route limit and the CHAT call's remaining caller timeout. When the resolved route is `crew_chat`, this call is an outbound data-flow edge to the Hermes crew's NAS chat endpoint (`cfg.crew_api_url`), independent of `askCrew`'s own Telegram edge. See `src/jarvis/llm/llm.spec.md`, "Chat backend selection".
- **Inputs**:
  - Redacted user query
  ...
  - **Warm profile block** (query-agnostic User + Directives excerpt from the knowledge graph, composed by `build_warm_profile()` / `format_warm_profile_block()` in [src/jarvis/memory/graph_ops.py](src/jarvis/memory/graph_ops.py) at Step 3.5 of `reply()`; no LLM call, pure SQLite read; injected unconditionally. A populated User branch renders the denial-mirroring heading, the real stored facts, and a rule making that list the only authority for user claims. An empty User branch renders `MEMORY_RECORD_COUNT: 0 (NONE RETRIEVED)`, forbids positive user-fact claims, and defines a stored-personal-context lookup as complete. Its response contract is only to disclose the empty result and ask for details the user wants remembered; identity discovery, external research, redirection, and persona banter are excluded. Anti-denial guidance exists only in the populated shape. Both shapes finish with the configured or mirrored reply-language decision and a no-switch rule. The result is cached in `DialogueMemory._hot_cache` under `DialogueMemory.WARM_PROFILE_CACHE_KEY` for the lifetime of the active conversation. Invalidated on `stop`, on new-conversation entry, AND on User/Directives graph mutations via the listener registered in [src/jarvis/daemon.py](src/jarvis/daemon.py) against `register_graph_mutation_listener` in [src/jarvis/memory/graph.py](src/jarvis/memory/graph.py); World-branch writes are ignored)
  ...
  - Tool schema: native via `generate_tools_json_schema()` ([src/jarvis/tools/registry.py](src/jarvis/tools/registry.py)) or text fallback via `_text_tool_call_guidance()` ([engine.py:68](src/jarvis/reply/engine.py:68)). The catalogue includes `systemManager` only when its independent default-off `system_management_enabled` opt-in is set; its schema exposes fixed operation enums and typed values, never a shell or registry passthrough.
  - Tool results from prior turns (raw or digested — see #5). Added memory
    provenance fields remain on local Python objects during ordinary turns;
    diary text keeps its existing date prefix for recency handling. When the
    user asks for an origin and `memoryProvenance` is invoked, its raw JSON
    result adds the relevant diary date, graph node and branch,
    vault-relative path, or Remio title to this CHAT context. That tool call
    and result are excluded from carryover, and a disclosed vault path is
    scrubbed before the assistant reply enters the hot window.
- **Conditional corrective input**: when the LLM router returned a narrowed selection containing at least one real tool but no grounding tool implementation has run, a prose response is appended to history but withheld from the user. One user-role `[Grounding required: ...]` instruction is appended for the next iteration. It identifies the router/execution mismatch, marks the prior claims unverified, requires a fitting tool call, and names `toolSearchTool` as the route when the current allow-list cannot perform or verify the work. This is a bounded extra iteration of this same CHAT context, not a separate LLM context. `stop` and `toolSearchTool` do not count as grounding executions; discovery must be followed by a real surfaced tool before external-result prose is accepted. Non-LLM selection strategies, a full-catalogue routing fallback, router `none`, and a memory-only plan do not activate the gate. A later prose response without a grounding tool becomes the localised honest fallback.
- **Output**: OpenAI-style `{content, tool_calls, thinking}`. Consumed by the tool orchestrator and TTS pipeline. Natural-language content is delivered when the malformed-output and structural zero-tool gates accept it; no post-turn evaluator runs. In text-tool mode, the syntax example names only a tool from the current allow-list. A `tool_calls:` protocol literal returned as content is malformed whether it is bare or follows a prose preface on a later line, and is blocked before delivery. When loop capacity remains, the engine withholds malformed content and makes one correction call requiring exactly one valid tool call or one grounded natural-language answer. A repeated malformed result, or one reached on the last loop turn, uses the localised canned fallback.
- **Streaming**: when the caller supplies `on_speech_segment`, `chat_with_messages()` passes an `on_token` listener and the request is streamed, so each finished sentence reaches the speech path while the rest of the reply is still being written. The assembled response is identical to the unstreamed one, so nothing downstream changes. Each loop turn gets its own `SpeechSegmenter`; a turn ending in a tool call drops its unfinished tail, and a stream that opens as structured output is never spoken. While a narrowed LLM-router selection signals external work and no grounding tool has run, the token listener is withheld so an ungrounded prose candidate cannot escape through TTS before the structural gate rejects it. See `src/jarvis/reply/reply.spec.md` (Speaking while writing).
...
- **Limits**: `num_ctx: 8192` (explicit). Timeout `llm_chat_timeout_sec` (180s default). A configured route chain's appended local CHAT candidate has the same 180-second route limit, while the caller may impose a smaller budget. With automatic crew handoff available, each main chat call is capped by the remaining 3-second decision budget, or the remaining 5-second hard budget when the structural close-to-done predicate is true. Router, embedding-router, planner, resolver, memory extractor, retrieval, and digest calls share the same applicable remainder. Native tools are attempted regardless of model size, except for Gemma-class SMALL models whose known pseudo-native output starts directly in text mode. HTTP 400 (`ToolsNotSupportedError`) switches any other model to text tool-calls and remains sticky for the session. Risk: `fetch_web_page` truncates at 50,000 chars (~37k tokens) — mitigated for SMALL models by tool-result digest (#5) which compresses the payload before it enters the messages history. LARGE models receive the raw payload and may silently see a truncated context.
...
- **Model / gating**: FAST tier — `resolve_model(cfg, Tier.FAST)` via `get_llm_backend(cfg).chat(...)`. The first enabled FAST route is authoritative for the effective model. `local_fast_model` (default `gemma4:e2b`) names the small local model the setup wizard provisions and budgets VRAM for; it is not a route, and FAST is answered by configured routes alone. The backend re-raises `ConnectionError` so the judge can apply a 30s cooldown after the server actively refuses; falls back to text-based wake detection while the cooldown is active.
- **Inputs**:
...
- **Output**: strict JSON `IntentJudgment{directed, query, stop, confidence, reasoning}` ([intent_judge.py:94](src/jarvis/listening/intent_judge.py:94)). Consumed by the listening state machine which dispatches to the reply engine. When `content` is empty **or truncated mid-JSON** (reasoning models count thinking tokens against the generation cap), the judge also recovers the JSON answer from `reasoning_content`; reasoning models typically end their thinking with the full structured answer. In hot-window and conversation modes, an extracted query that matches the previous TTS while the newly heard text does not is discarded in favour of the heard text, preventing a judge echo-selection error from replaying Jarvis's own answer as a user request.
...
- **Model / gating**: FAST tier — `resolve_model(cfg, Tier.FAST)`. Factory-dispatched.
- **Inputs**: user query, tool catalogue (builtin + MCP with descriptions), optional narrow-down hint. Default-off builtins such as `systemManager`, `browserInteract`, and `desktopInteract` enter this catalogue only while their respective opt-in setting is enabled. User-prompt order is KV-cache-disciplined: the mostly-static catalogue opens, the dynamic hint (time + dialogue) follows, the query is the final token — consecutive router calls in one conversation share the full catalogue as prefix.
- **System prompt**: inline (~lines 260-315). Teaches pick up-to-5 tools or `none`.
- **Output**: comma-separated tool names or `none`, followed by one classification word (`LOCAL`, `COMPLEX`, or `HERMES`) naming which Tier.CHAT backend the turn prefers. Tool names are capped at `_LLM_MAX_SELECTED` (5); always-included tools (`stop`, `toolSearchTool`) are unioned in regardless. The classification word rides the same response rather than a second call: when the caller passes `chat_backend_signal`, it is populated with `{"preference": "local" | "complex" | "hermes"}` and reused by the main reply loop's backend selection (#1) while `chat_backend_override` is `"auto"`. `local` resolves to no provider preference, because the cheapest configured route is the chain order already. See `src/jarvis/tools/selection.spec.md`, "Chat backend preference".
- **Limits**: `llm_timeout_sec`. Local route limit 60 seconds, with any smaller caller budget taking precedence. `num_ctx: 8192` keeps an Ollama model shared by FAST and CHAT on the same resident runner as the main loop instead of rebuilding it between 4096 and 8192 contexts. `max_tokens: 50` — the cap describes the answer, and a route whose model spends it all on reasoning instead is asked again with `REASONING_ANSWER_ALLOWANCE` (1024) added by `OpenAICompatibleBackend.direct()`, which applies to every FAST context rather than this one (see `src/jarvis/llm/llm.spec.md`, "Reasoning budgets"). On failure → keyword strategy, and the classification signal is left unset (fail-open); every such fallback also prints a user-visible line, because a router failing on *every* turn otherwise looks exactly like one that is working.
- **Caching**: `routed_tools` and the chat-backend classification are cached together in `DialogueMemory._hot_cache` under key `router:{redacted_query}|{strategy}|{builtin-names}|{mcp-names}` for the lifetime of the active conversation, since both came from the same response. For the embedding strategy, each static tool-description vector is also held in a bounded process cache keyed by backend endpoint, model, tool name, and summary. Voice startup warms that catalogue; live turns normally embed only the query. The dialogue catalogue signature lets a mid-conversation MCP refresh invalidate the routed selection, while a changed tool summary creates a new static-vector key.
...
- **Trigger**: after each daily summary (#9), and when the user explicitly runs the bounded school-note import. The diary path is background; the school-note path is never automatic.
...
- **Exam countdown** ([src/jarvis/tools/builtin/exam_countdown.py](src/jarvis/tools/builtin/exam_countdown.py)) uses the FAST tier via `resolve_model(cfg, Tier.FAST)`. It runs only when `getExamCountdown` is invoked and the School branch is non-empty. The input is a bounded JSON snapshot of populated School nodes fenced as untrusted data. The output is a JSON array of subject, exact recorded date text, and an advisory ISO date. A deterministic validator rejects an ISO candidate unless the exact date text occurs in the source and supplies standalone day and four-digit year evidence. `max_tokens: 600`, temperature 0, and `llm_tools_timeout_sec` bounded by the reply deadline. Empty branches skip the call.
...
## 17b. Morning School Briefing

- **File**: [src/jarvis/memory/morning_briefing.py](src/jarvis/memory/morning_briefing.py) through `generate_morning_briefing()`.
- **Trigger**: at or after `morning_briefing_time`, at most once per local day, from the existing `AmbientDigestWorker`. The feature defaults off. A persisted local delivery date suppresses repeats and a multi-day gap never creates replay calls. Busy listening, speaking, conversation, command-capture, hot-window, and query states defer before generation. State is checked again before TTS queueing. An empty School branch skips generation and is cached as empty for the process-local day.
- **Model / gating**: CHAT tier via `resolve_model(cfg, Tier.CHAT)`. Gated by `morning_briefing_enabled`, the local trigger time, persisted delivery state, and idle voice/query state.
- **Inputs**: the query-independent reply prefix, including unified persona and voice-language instruction; today's local ISO date; and the same bounded School-branch snapshot used by the exam tool, fenced as untrusted data.
- **System prompt**: `build_reply_prompt_prefix(cfg)` followed by static briefing rules. It asks for plain spoken sentences under one minute, useful school priorities, no markdown, and no invented or silently corrected dates. Python does not assemble language-specific sentence fragments.
- **Output**: one natural-language briefing queued through the configured TTS engine. A generated briefing deferred by a newly active conversation stays cached for that local day. Empty output or a failed model/TTS call does not advance the persisted delivery gate.
- **Limits**: `llm_chat_timeout_sec`; `max_tokens: 350`; one call per delivered local day plus retries after failures. School data follows the configured CHAT route and can therefore reach an enabled remote CHAT provider.
...
- Routes and models: `llm_routes` contains ordered FAST and CHAT entries. `llm_chat_model` and `fast_model` are derived from the first enabled routes for prompt sizing. `local_fast_model` names the wizard-provisioned local model and backs no route; `ollama_chat_model` is the PRIVATE model. Local embeddings use `ollama_embed_model`. Every explicit context model is obtained through `resolve_model(cfg, tier)`, except the untiered ambient digest (#17), which calls `cfg.llm_chat_model` directly. Streaming uses one monotonic request deadline; an available loopback route receives a 1.2-second progress window before the remaining tier chain is tried, and the first route to emit text exclusively owns the answer.
- Embeddings: `ollama_embed_model` through loopback `get_embedding_backend(cfg)`. Embeddings never use `llm_routes`.
- Flags: `memory_digest_enabled`, `tool_result_digest_enabled`, `remio_memory_enabled`, `llm_thinking_enabled`, `intent_judge_thinking_enabled`, `tool_selection_strategy`, `planner_enabled`, `low_power_mode`, `passive_capture_enabled`, `morning_briefing_enabled`, `computer_interaction_enabled`, `system_management_enabled`. Enabled Remio retrieval runs locally alongside planner-directed diary lookup, contributes attributable excerpts only, and fails without changing the prompt. Computer interaction defaults off and controls whether the two semantic interaction tools are registered at all. System management defaults off independently and controls whether `systemManager` is registered and therefore visible to the router, planner, and main reply loop.
- Chat backend selection: `chat_backend_override` (`"auto"` default, or a route provider name such as `"openai_compatible"`, `"claude_subscription"`, `"codex_subscription"`, or `"crew_chat"`) **pins** the main reply loop's Tier.CHAT call (#1) to that backend: it answers or the turn fails, with no other route tried. Under `"auto"`, the tool router's per-turn classification (#7, `LOCAL`/`COMPLEX`/`HERMES`) supplies a preference instead, which reorders the chain but still falls through on failure; `LOCAL` names no provider, since there is no local reply model. Neither adds an LLM call. `crew_chat_agent` names which crew specialist a `crew_chat` route asks. See `src/jarvis/llm/llm.spec.md`, "Chat backend selection", "Codex subscription session", and "Crew chat relay".
- Timeouts: `llm_chat_timeout_sec` (180s, also #17 and #17b), `llm_chat_chain_budget_sec` (30s, the ceiling on one walk of a chat route chain across every route it tries, so several individually reasonable routes cannot add up to longer than a spoken turn can wait), `llm_digest_timeout_sec` (8s, shared across #4/#5/#6), `llm_tools_timeout_sec`, `intent_judge_timeout_sec` (6s), `planner_timeout_sec` (3s), `simple_reply_first_audio_sec` (3s), `memory_reply_first_audio_sec` (10s), `passive_digest_interval_min` (ambient cadence on the shared worker). Reply budgets are monotonic and planner-directed memory work shares the remaining budget. When automatic crew handoff is enabled and the existing crew token and chat ID are configured, the current `TurnTrace` additionally caps local pre-flight and loop work at the 3-second decision point or 5-second hard cutoff. These handoff thresholds are the fixed reply contract, not configuration keys.
...
local morning tick (briefing switch on, user path idle)
  └─▶ [17b] School briefing → TTS queue
```
...
6. Consider single-model deployments: the effective FAST route can reuse the CHAT route, and PRIVATE work reuses `ollama_chat_model`; loading a second local model hurts cold-start latency on small hardware.
...
- **Limits**: provider warmup uses the shared 60 s startup budget. Reply-prefix prefill uses the same remaining per-thread timeout, `max_tokens: 1`, no tools, and thinking disabled; residency comes from the route the backend was built with, so low power mode applies to the prefill too.
