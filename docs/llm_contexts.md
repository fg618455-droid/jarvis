...
- **Model / gating**: CHAT tier. The route-specific model and timeout come from the ordered CHAT chain. Not optional. No size branching on the loop itself; size branching affects the digests around it. Which route answers first within that chain can be biased per turn: `cfg.chat_backend_override` (`"auto"` by default, or a specific route provider) and, under `"auto"`, the tool router's own classification (see #7) resolve to a `preferred_provider` passed to `RoutedBackend.chat()`, which only reorders its existing chain for that one call — an unavailable or failing preferred route still falls through to the normal order. See `src/jarvis/llm/llm.spec.md`, "Chat backend selection".
- **Inputs**:
  - Redacted user query
...
- **Output**: comma-separated tool names or `none`, followed by one classification word (`LOCAL` or `COMPLEX`) naming how much reasoning the turn needs. Tool names are capped at `_LLM_MAX_SELECTED` (5); always-included tools (`stop`, `toolSearchTool`) are unioned in regardless. The classification word rides the same response rather than a second call: when the caller passes `chat_backend_signal`, it is populated with `{"preference": "local" | "complex"}` and reused by the main reply loop's backend selection (#1) and its `"auto"` `chat_backend_override`. See `src/jarvis/tools/selection.spec.md`, "Chat backend preference".
...
- Chat backend selection: `chat_backend_override` (`"auto"` default, or a route provider name) forces the main reply loop's Tier.CHAT call (#1) to a specific backend; under `"auto"`, the tool router's per-turn classification (#7) picks instead. Neither adds an LLM call, and an unavailable or failing preferred backend falls through to the normal CHAT chain unchanged. See `src/jarvis/llm/llm.spec.md`, "Chat backend selection".
