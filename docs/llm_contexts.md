...
- **Model / gating**: FAST tier — `resolve_model(cfg, Tier.FAST)` via `get_llm_backend(cfg).chat(...)`. The first enabled FAST route is authoritative for the effective model. `local_fast_model` (default `gemma4:e2b`) is a separate Ollama fallback and is never replaced by a remote provider's model identifier. The backend re-raises `ConnectionError` so the judge can apply a 30s cooldown after the server actively refuses; falls back to text-based wake detection while the cooldown is active.
- **Inputs**:
...
- Routes and models: `llm_routes` contains ordered FAST and CHAT entries. `llm_chat_model` and `fast_model` are derived from the first enabled routes for prompt sizing. `local_fast_model` is the Ollama FAST fallback; `ollama_chat_model` is the CHAT fallback and PRIVATE model. Local embeddings use `ollama_embed_model`. Every explicit context model is obtained through `resolve_model(cfg, tier)`, except the untiered ambient digest (#17), which calls `cfg.llm_chat_model` directly. Streaming uses one monotonic request deadline; an available loopback route receives a 1.2-second progress window before the remaining tier chain is tried, and the first route to emit text exclusively owns the answer.
...
- Chat backend selection: `chat_backend_override` (`"auto"` default, or a route provider name such as `"ollama"`, `"claude_subscription"`, `"codex_subscription"`, or `"crew_chat"`) forces the main reply loop's Tier.CHAT call (#1) to a specific backend; under `"auto"`, the tool router's per-turn classification (#7, `LOCAL`/`COMPLEX`/`HERMES`) picks instead. Neither adds an LLM call, and an unavailable or failing preferred backend falls through to the normal CHAT chain unchanged. `crew_chat_agent` names which crew specialist a `crew_chat` route asks. See `src/jarvis/llm/llm.spec.md`, "Chat backend selection", "Codex subscription session", and "Crew chat relay".
...
6. Consider single-model deployments: the effective FAST route can reuse the CHAT route, while the local fallback can separately reuse `ollama_chat_model`; loading a second local model hurts cold-start latency on small hardware.
