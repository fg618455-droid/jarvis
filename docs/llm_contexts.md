...
- **Model / gating**: CHAT tier. The route-specific model and timeout come from the ordered explicit cloud/subscription chain. `cfg.chat_backend_override` is `"auto"` or a configured non-Ollama provider; the tool router's `DEFAULT`, `COMPLEX`, or `HERMES` signal may reorder that chain for one turn. Legacy `LOCAL` is accepted as `DEFAULT`, never as an Ollama route. An unavailable preferred route falls through to normal configured order; complete exhaustion returns the fixed honest user-facing failure. Subscription sidecar/process and crew relay details are specified in `src/jarvis/llm/llm.spec.md`.
- **Inputs**:
  - Redacted user query
...
- **Limits**: Timeout `llm_chat_timeout_sec` (180s default), capped by the caller's shared deadline. There is no appended local CHAT candidate. Native tools are attempted on routes that claim them; `ToolsNotSupportedError` switches the reply engine to the text-tool parser/dispatcher without changing privacy lanes.
...
- **Model / gating**: FAST tier — the first enabled, credentialed cloud FAST route is authoritative. There is no local FAST model or Ollama fallback. Complete exhaustion is recorded and the judge falls back to deterministic text-based wake detection.
- **Inputs**:
...
- **Model / gating**: PRIVATE tier via `resolve_model(cfg, Tier.PRIVATE)`. Gated by `memory_digest_enabled`; the auto-on decision still follows the effective CHAT model size.
...
- **Model / gating**: PRIVATE tier via `resolve_model(cfg, Tier.PRIVATE)`. Gated by `tool_result_digest_enabled`; auto-on remains based on `detect_model_size(cfg.llm_chat_model)`.
...
- **Output**: comma-separated tool names or `none`, followed by `DEFAULT`, `COMPLEX`, or `HERMES`. `DEFAULT` keeps configured CHAT order, `COMPLEX` prefers Claude subscription, and `HERMES` prefers crew chat. Legacy `LOCAL` parses as `DEFAULT` for transition compatibility.
- **Limits**: `llm_timeout_sec`, capped by the turn deadline; `max_tokens: 50`. On failure → keyword strategy and no preference signal (fail-open).
...
- **Model**: PRIVATE tier. The fact and category labels stay on loopback Ollama; caller-supplied FAST/CHAT picker models cannot override this boundary.
...
- **Model / gating**: `resolve_model(cfg, Tier.PRIVATE)` via `get_llm_backend(cfg).direct(...)`, using the local PRIVATE chain. Gated by the live `passive_capture_enabled` switch. Addressed and already-digested lines are excluded before the call.
...
| 4 | Memory digest | 0-N | auto by size | PRIVATE |
| 5 | Tool-result digest | 0-N | auto by size | PRIVATE |
...
| 11 | Graph best-child | 0-N | No (background) | PRIVATE |
...
| 17 | Ambient digest | 0-1 per configured interval | passive capture only | PRIVATE (local Ollama) |
...
- Routes and models: `llm_routes` contains ordered cloud FAST and cloud/subscription CHAT entries. `llm_chat_model` and `fast_model` are derived from the first valid routes for prompt sizing. `ollama_chat_model` is PRIVATE-only and `ollama_embed_model` is the local embedding model. Every explicit context uses `resolve_model(cfg, tier)`, including the ambient digest. Streaming uses one monotonic request deadline and configured order; the first route to emit text exclusively owns the answer.
...
- Chat backend selection: `chat_backend_override` (`"auto"` default or an explicit configured cloud/subscription provider) biases the main CHAT call. Under `"auto"`, `DEFAULT`/`COMPLEX`/`HERMES` comes from the tool-router response. `ollama` is rejected and legacy `LOCAL` means `DEFAULT`. An unavailable preferred route falls through to configured CHAT order. `crew_chat_agent` names the crew specialist for a `crew_chat` route.
...
6. FAST and CHAT may use the same explicit cloud endpoint. PRIVATE uses `ollama_chat_model`; neither cloud lane adds a local fallback.
...
- **Model / gating**: the first available candidate in the requested route lane. Only PRIVATE and embedding warm-up may contact local Ollama. Reply-prefix prefill uses CHAT after its configured provider warm-up succeeds.
...
Runtime snapshots copy settings and include resolved environment credentials in a
process-salted fingerprint. Credential rotation takes effect on the next installed
generation; existing turns keep their previously constructed provider adapters.

Graph category selection and school-note import are PRIVATE contexts, including
control-centre import entry points. The graph LLM boundary enforces PRIVATE even
when a caller supplies a model carrying a different lane.
