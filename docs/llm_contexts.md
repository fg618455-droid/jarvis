...
- Chat backend selection: `chat_backend_override` (`"auto"` default, or a route provider name — `"ollama"`, `"claude_subscription"`, `"crew_chat"`) forces the main reply loop's Tier.CHAT call (#1) to a specific backend; under `"auto"`, the tool router's per-turn classification (#7, `LOCAL`/`COMPLEX`/`HERMES`) picks instead. Neither adds an LLM call, and an unavailable or failing preferred backend falls through to the normal CHAT chain unchanged. `crew_chat_agent` names which crew specialist a `crew_chat` route asks. See `src/jarvis/llm/llm.spec.md`, "Chat backend selection" and "Crew chat relay".
