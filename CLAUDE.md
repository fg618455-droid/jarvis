...
Jarvis's voice pipeline (wake word, speech-to-text, text-to-speech) is offline-first by principle and stays fully local, no exceptions. Do not add a cloud STT/TTS vendor (e.g. ElevenLabs, Fish Audio, or any SaaS that requires shipping a recording of what was said at home to a third party's servers), not even as an opt-in feature — decline on principle and redirect to better local voices (Piper, Chatterbox TTS) instead.

The reasoning/LLM backend is a separate decision (2026-08-18, Felix): simple, fast turns default to the local model as before, but a complex, slow, or orchestration-heavy turn (e.g. one that needs to read through the memory graph, coordinate several tools, or would just take too long locally) may route to the self-hosted Hermes agent crew, which runs under Felix's own already-paid ChatGPT/Codex subscription — see `askCrew`, Phase 7 of the Mission Control plan (`docs-felix/MISSION-CONTROL-PLAN.md`, not versioned in this repo). Cost was the original reason for local-only on the reasoning path, and it no longer applies since the subscription is paid for regardless of use. This carve-out is scoped to that one already-considered path; any other new cloud vendor integration still needs to be raised explicitly, not assumed.
...
| `src/jarvis/diagnostics.spec.md` | Local capability reports, redacted JSONL events, MCP preflight | Non-destructive checks; no prompts, payloads, or credentials; logging is fail-soft |
...
| `src/jarvis/listening/listening.spec.md` | Voice listener, wake word detection, audio pipeline, streamed speech queue | Each utterance carries its own callbacks; a streamed reply is closed with an end-of-reply marker |
| `src/jarvis/output/tts.spec.md` | TTS engine selection (Piper/Chatterbox/Kokoro), the Kokoro sidecar subprocess: protocol, lifecycle, error handling | AGPL-licensed Kokoro code runs only inside its own sidecar process, never imported by the main daemon; a sidecar failure degrades to a skipped utterance, never a raw exception |
...
| `src/jarvis/reply/reply.spec.md` | LLM reply generation, tool use, profiles, speaking while writing | Tools return raw data; profiles handle formatting; a failing speech path costs the sound, never the answer |
...
| `src/jarvis/tools/builtin/ask_crew.spec.md` | askCrew tool: fire-and-forget delegation to the self-hosted Hermes crew over Telegram | Explicit and gated, not a default fallback; never waits for or reads back a reply; no new privileged NAS surface |
| `src/jarvis/tools/tools.spec.md` | Stable tool errors, SSRF-safe page fetching, screenshot capture, MCP package pins | User-safe failures; every redirect is validated; optional dependencies fail explicitly |
...
| `src/jarvis/tools/builtin/open_on_computer.spec.md` | openOnComputer tool: opens websites, applications and home-directory paths on the user's own machine | Never touches a shell; http/https only; applications resolve to an absolute executable before launch, with no arguments |
...
| `src/jarvis/telegram/telegram.spec.md` | Single Bot API update router, Telegram as a conversation channel, confirmation delivery | One poller per token or updates are lost; the polling thread never waits on a reply; conversation is opt-in |
...
| `src/jarvis/memory/vault/vault.spec.md` | Obsidian vault memory: read-only index over the whole vault, one-folder mirror of graph nodes as markdown with frontmatter and wikilinks | Read broad, write fenced to one folder; SQLite stays authoritative; dry-run by default; user content below the end marker is never touched |
