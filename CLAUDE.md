...
| `src/jarvis/webui/webui.spec.md` | Control centre: the face-centred deck, its widgets and panels, themes, request guards, standalone mode | One process so views read live objects; the face is the page and a detail opens beside it, never instead of it; Settings is the only separate destination; a theme is paint and never structure; offline, no build step; a failed start never stops the daemon |
...
| `src/desktop_app/headless_launcher.spec.md` | Splash screen + daemon subprocess with no tray, chat window, or face widget | Reuses the tray's readiness checks unmodified; the control centre owns opening the browser, not this launcher |
