...
const AV = (() => {
  ...
  /* ------------------------------ bus polling ------------------------------ */
  // Jarvis serves this face inside its control centre rather than from
  // ai-visualizer's own server.py, so the bus lives at /api/visualizer/*
  // instead of the stock /state and /config paths.
  let raw = { state: "idle", level: 0, samples: null, alert: false,
              loading: false };
  if (!DEMO) {
    setInterval(async () => {
      try {
        const r = await fetch("/api/visualizer/state", { cache: "no-store" });
        raw = await r.json();
  ...
  A.init = (opts = {}) => {
    ...
    if (DEMO) {
      ...
      fetch("/api/visualizer/config", { cache: "no-store" })
