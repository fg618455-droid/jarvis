...
database directly. A NAS that is off or unreachable degrades the view to
"nothing to show" instead of a broken page.

The activity feed (``GET /api/crew``) only ever reads, and its reading is
shaped here rather than in the browser, because the crew poller publishes
the same shape over the event bus and both must agree. The chat relay
