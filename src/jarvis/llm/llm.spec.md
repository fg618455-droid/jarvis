...
A tier's chain is not one list. A call carrying a tool schema considers only routes advertising `tools`, which can be far shorter than the tier's own chain, so a configuration whose chat routes mostly cannot carry tools can read every request and perform none of them once the few that can are in cooldown. That walk ends before it starts: nothing is asked, no route is marked, and the outcome is indistinguishable from a chain whose routes all failed. `debug_log` therefore names the tier and the capability whenever a call finds no candidate at all.
