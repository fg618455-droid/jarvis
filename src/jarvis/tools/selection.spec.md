...
Every one of those fallbacks also prints a user-visible line naming why the router gave up, alongside its diagnostic log entry. One fallback is fail-open and unremarkable. A router falling back on *every* turn is a fault with no symptom of its own: replies still arrive, but the chat model sees an unfiltered catalogue each time and the reply engine's zero-tool grounding gate stops activating, because a full catalogue reads as a fallback shape rather than a relevance signal (see `../../reply/reply.spec.md`). Nothing else in a running system distinguishes that from ordinary operation, so the router says so where a person watching the run will see it rather than only where it must be gone looking for. A `"none"` decision is the router working and stays silent.
...
The token is removed from the tool-list text for every caller, whether or not this one asked for the classification. The router answers the same way either way, so `"none LOCAL"` has to read as a positive no-tool decision regardless; letting an unrelated argument change how the tool list parses would make the classification a second, hidden contract.

The *value* is only surfaced when the caller passes `chat_backend_signal`, a dict the router populates with `{"preference": "local" | "complex" | "hermes"}`. The key is left absent whenever no classification token is found in the response (including every fallback path in step 7): an absent key is the caller's fail-open signal to leave backend selection at its existing default. Other strategies never populate this dict.
